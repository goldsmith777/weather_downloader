...
def dbconfig_load():
    rawData = open('dbconfig_test.txt', 'r').readlines()
    dbconfig = {}
    for item in rawData:
       dbconfig = dict(dbconfig,**eval(item))
    # print dbconfig
    return dbconfig
...
def forecast4d_response_trans(response, allPara, weatherCondition, windDire, windLevel, day):
    ...
    try:
        ...
        print "Can't get the forecast weather"
    return result
...
def index_response_trans(response):
    ...
    try:
        ...
        print "Can't get the index weather"
    return result
...
def observe_response_trans(response, allPara, weatherCondition, windDire):
    ...
    try:
        ...
        print "Can't get the observe weather"
    return finalResult
...
# 将数据写入数据库
def insert_DB(content,dbconfig):
    # connStr = "host='" + dbconfig['host'] + "',user='" + dbconfig['user'] + "',passwd='" + dbconfig['passwd'] \
    #            + "',db='" + dbconfig['db'] + "',port=" + dbconfig['port']+",charset='" + dbconfig['charset'] + "'"
    # connStr = "host='" + dbconfig['host'] + "',user='" + dbconfig['user'] + "',passwd='" + dbconfig['passwd'] \
    #           + "',db='" + dbconfig['db'] + "'"
    # print connStr,type(connStr)
    try:
        conn = mdb.connect(host=dbconfig['host'], user=dbconfig['user'], passwd=dbconfig['passwd'], db=dbconfig['db']
        ...
        forecast_sql = "INSERT INTO " + dbconfig['table'] + "(" + keys[:-1].replace('\'', '') + ") VALUES (" + values[:-1] + ")" \
        ...
        if content.has_key('hourWeather') and content.has_key("observeTime"):
            observe_sql = "INSERT INTO " + dbconfig['table'] + "(" + observe_keys + observe_sql_keys + ") VALUES (" \
                          + observe_values + observe_sql_values + ")" + " ON DUPLICATE KEY UPDATE " + observe_duplicate_key_values + hourWeatherUpdate
            cur.execute(observe_sql)
        if content.has_key("airTime"):
            air_sql = "INSERT INTO " + dbconfig['table'] + "(" + air_keys + air_sql_keys + ") VALUES (" + air_values + air_sql_values + ")" \
        ...
        try:
            ...
            for i in range(7):
                ...
                if i == 0:
                    ...
                    # print observe_contentDB
                    # prin_t air_contentDB
                else:
                    contentDB = forecast4d_contentDB
                contentDB['countyID'] = cityID
                insert_DB(contentDB,dbconfig)
        except Exception, e:
            print "httpRequest timeout!"
            print e.message
        # break
runTime = datetime.now() - startTime
print "runTime: {0} ".format(runTime)
# print ("finish %s", datetime.strftime(datetime.now(),"%Y%m%d %H:%M:%S"))